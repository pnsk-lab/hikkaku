...
const descriptorEquals = (a: InputLink | undefined, b: InputLink | undefined) => {
...
const isStatementInputName = (inputName: string) => /^SUBSTACK\d*$/.test(inputName);
...
const buildBlockNode = (
	...
	for (const [fieldName, fieldSource] of Object.entries(source.fields)) {
		...
		nodeChildren.push(createTreeNode("field", fieldAttrs, [String(field.value)]));
	}
...
export const stripUnsupportedNextNodes = (
	...
	for (const node of Array.from(blockNodes)) {
		...
		try {
			...
			const mutation = readMutationFromBlockElement(node as unknown as XmlLikeNode);
